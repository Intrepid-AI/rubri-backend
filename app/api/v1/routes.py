...
async def _process_file_upload(
    ...
    if file_ext not in Constants.ALLOWED_FILE_TYPES.value:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported file type: {file_ext}. Supported types: {', '.join(Constants.ALLOWED_FILE_TYPES.value.keys())}"
        )
